#!/bin/bash
#
# Builds distributable zip files for JasAI skills.
...
#     └── jasai-skills-marketplace.zip   All departments in one zip
#
...
(cd "$ROOT" && zip -r "$DIST/extension/jasai-skills-marketplace.zip" .claude-plugin/marketplace.json plugins/ .env.example -x "*/\.*" > /dev/null 2>&1)
SIZE=$(du -h "$DIST/extension/jasai-skills-marketplace.zip" | cut -f1 | xargs)
echo "  Created extension/jasai-skills-marketplace.zip ($SIZE)"
